...
class AssetsAudioPlayer {
  ...
  Map<String, dynamic> get getCurrentAudioextra => _current.value?.audio.audio.metas.extra ?? <String, dynamic>{};
  ...
  set showNotification(bool newValue) {
    ...
    /* await */ _sendChannel.invokeMethod('showNotification', {'id': id, 'show': _showNotification});
  }
  ...
  Future<bool> previous({bool keepLoopMode = true}) async {
    if (_playlist != null) {
      ...
      if (_currentPosition.valueOrNull != null && _currentPosition.valueOrNull!.inSeconds >= 5) {
  ...
  void _handleOnError(Map args) async {
    ...
    if (onErrorDo != null) {
      final errorHandler = ErrorHandler(player: this, currentPosition: currentPosition.value, playlist: _playlist?.playlist, playlistIndex: _playlist?.playlistIndex, error: error);
      if (onErrorDo != null) {
        onErrorDo!(errorHandler);
      }
    }
    ...
    if (audioInput != null) {
      ...
      try {
        final params = {
          ...
          'pitch': pitch ?? audio.pitch ?? this.pitch.valueOrNull ?? defaultPitch,
        };
        if (seek != null) {
